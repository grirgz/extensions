...
ParaTimeline {
	...
	*new { arg w, bounds; 
		...
		mouseTracker
			...
			.mouseDownAction_({|me, x, y, mod, buttonNumber, clickCount|
				...
				if( (mod & 0x00040000) != 0, {	// == 262401
					//paraNodes.add(ParaTimelineNode.new(x,y, fillcolor, bounds, nodeCount, nodeSize));
	...
	createNode {arg x, y, color, refresh=true;
		...
		paraNodes.add(ParaTimelineNode.new(x+0.5, y+0.5, fillcolor, bounds, nodeCount, nodeSize, nodeAlign));
		nodeCount = nodeCount + 1;
		if(refresh == true, {this.refresh});
	}
...
//		y = argY+bounds.top;
	...
	setNodeStates_ {arg array; // array with [locs, connections, color, size, string]
		if(array[0].isNil == false, {
			paraNodes = List.new; 
			array[0].do({arg loc; 
				paraNodes.add(ParaTimelineNode.new(loc.x, loc.y, fillcolor, bounds, nodeCount, nodeAlign));
				nodeCount = nodeCount + 1;
				})
		});
...
ParaTimelineNode {
	...
	*new { arg x, y, color, bounds, spnum, size, align=\topLeft; 
	...
	compute_rect { arg point;
		//align.debug("ParaTimelineNode: align");
		switch(align,
			\centerLeft, {
				//align.debug("ParaTimelineNode1: align");
				rect = Rect(nodeloc.x+0.5, nodeloc.y-(size/2)+0.5, len ?? size, size);
			},
			\center,  {
				//align.debug("ParaTimelineNode2: align");
