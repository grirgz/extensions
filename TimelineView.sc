	...
	*new { arg w, bounds; 
	...
	initParaSpace { arg w, argbounds;
		...
		mouseTracker
			...
			.mouseDownAction_({|me, wx, wy, mod, buttonNumber, clickCount|
				var bounds = this.bounds;
				var x, y;
				x = wx/bounds.width;
				y = wy/bounds.height;
				mouseDownAction.(me, x, y, mod, buttonNumber, clickCount);
			...
			.mouseMoveAction_({|me, wx, wy, mod|
				var bounds = this.bounds;
				var x,y;
				x = wx/ bounds.width;
				y = wy/ bounds.height;
				mouseMoveAction.(me, x, y, mod);
				if(chosennode != nil, { // a node is selected
					relX = chosennode.nodeloc.x - bounds.left - 0.5;
					relY = chosennode.nodeloc.y - bounds.top - 0.5;
			...
			.drawFunc_({		
				...
				var bounds = this.bounds;
				var wstartSelPoint, wendSelPoint;
				...
				// the selection node
				wstartSelPoint = startSelPoint * bounds.extent;
				wendSelPoint = endSelPoint * bounds.extent;
				pen.fillRect(Rect(	wstartSelPoint.x + 0.5, 
									wstartSelPoint.y + 0.5,
									wendSelPoint.x - wstartSelPoint.x,
									wendSelPoint.y - wstartSelPoint.y
									));
				pen.color = selectStrokeColor;
				pen.strokeRect(Rect(	wstartSelPoint.x + 0.5, 
									wstartSelPoint.y + 0.5,
									wendSelPoint.x - wstartSelPoint.x,
									wendSelPoint.y - wstartSelPoint.y
									));
	...
	createNode {arg x, y, size, color, refresh=true;
		var bounds = this.bounds;
		this.createNode1(x/areasize.x, y/areasize.y, size/areasize.x, color, refresh);
	}
...
//		y = argY+bounds.top;
	...
	setNodeStates_ {arg array; // array with [locs, connections, color, size, string]
		...
		if(array[0].isNil == false, {
			paraNodes = List.new; 
			array[0].do({arg loc; 
				paraNodes.add(TimelineViewNode.new(loc.x, loc.y, fillcolor, bounds, nodeCount, nodeAlign));
				nodeCount = nodeCount + 1;
				})
		});
...
TimelineViewNode {
	...
	*new { arg x, y, color, bounds, spnum, size, align=\topLeft; 
	...
	compute_rect { arg point;
		...
		switch(align,
			\centerLeft, {
				...
				rect = Rect(nodeloc.x+0.5, nodeloc.y-(size/2)+0.5, len ?? size, size);
			},
			\center,  {
				//align.debug("TimelineViewNode2: align");
				rect = Rect(nodeloc.x-(size/2)+0.5, nodeloc.y-(size/2)+0.5, len ?? size, size);
			},
			\debug,  {
				//align.debug("TimelineViewNode2: align");
				rect = Rect(nodeloc.x, nodeloc.y, len ?? size, size);
			},
			{
				rect = Rect(nodeloc.x+0.5, nodeloc.y+0.5, len ?? size, size);
			}
		);
	}
	...
	initGridNode {arg argX, argY, argcolor, argbounds, spnum, argsize, argalign;
		...
		size = argsize ? 8;
		bounds = argbounds;
		//rect = Rect((argX-(size/2))+0.5, (argY-(size/2))+0.5, len ?? size, size);
		align = argalign;
		this.compute_rect;
		string = "";
		temp = nil;
